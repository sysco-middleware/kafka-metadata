...
import no.sysco.middleware.kafka.metadata.topic.rest.{ Team, TopicMetadata, TopicMetadataJsonProtocol }
import no.sysco.middleware.kafka.metadata.topic.utils.Utils
  ...
  "TopicVendorProtocol in use, " when {
    ...
    "json => pojo. " should {
      ...
      "Team return proper json " in {
        val json = Utils.jsonFromFile("topic/src/test/resources/team.json")
      ...
      "TopicMetadata return proper pojo " in {
        val json = Utils.jsonFromFile("topic/src/test/resources/topic-metadata.json")
